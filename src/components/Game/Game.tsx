...
const Game = React.memo(() => {
	...
	const [wonOnGuess, setWonOnGuess] = useState<number>(0);
	...
	const updateStatisticsState = (didWin: boolean) => {
		...
		let gamesFailed: number = didWin
			? statistics.guesses.fail
			: statistics.guesses.fail + 1;
		let gamesWon: number = didWin
	...
	return (
		<div className='game-container'>
			...
			<Keys
				keys={keys}
				handleKeyInput={(keyVal: string) => handleKeyInput(keyVal)}
			/>
